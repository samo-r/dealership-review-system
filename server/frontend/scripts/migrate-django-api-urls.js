/**
 * One-time codemod: route all /djangoapp fetch URLs through djangoApiUrl().
...
const UTIL_PATH = path.join(SRC_DIR, "utils", "djangoApi.js");
...
function ensureImport(content, fromFile) {
  if (!content.includes("djangoApiUrl")) {
    return content;
  }
  if (/from\s+["'].*djangoApi["']/.test(content)) {
    return content;
  }

  const importLine = `import { djangoApiUrl } from "${importPath(fromFile)}";\n`;
...
for (const file of walk(SRC_DIR)) {
  if (file.replace(/\\/g, "/").endsWith("utils/djangoApi.js")) {
    continue;
  }
