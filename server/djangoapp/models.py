...
# This replaces the default auth.User and must be set as AUTH_USER_MODEL
# in settings.
class User(AbstractUser):
    ...
    def clean(self):
        """Enforce role and dealer-assignment rules before saving."""
        super().clean()
        ...
        if (
            self.role == self.Roles.DEALER_ADMIN
            and not self.assigned_dealer_id
        ):
            raise ValidationError(
                {
                    "assigned_dealer_id": (
                        "Dealer Admin must have an assigned dealer ID."
                    ),
                }
            )
